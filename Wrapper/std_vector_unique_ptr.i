//Credits:
//https://github.com/swig/swig/blob/v4.0.1/Lib/java/std_vector.i
...
%define SWIG_STD_VECTOR_MINIMUM_INTERNAL_UNIQUE(CTYPE, CONST_REFERENCE)
/*
...
%}
  ...
  public:
    ...
    %extend {
      ...
      /*
      vector(jint count, CTYPE &value) throw (std::out_of_range) {
        if (count < 0)
      ...
      */
      ...
      /*
      void doAdd(value_type& x) {
        self->push_back(std::move(x));
      }
      */

      /*
      void doAdd(jint index, value_type& x) throw (std::out_of_range) {
        jint size = static_cast<jint>(self->size());
        if (0 <= index && index <= size) {
          self->insert(self->begin() + index, std::move(x));
        } else {
      ...
      */
      ...
      CTYPE doRemove(jint index) throw (std::out_of_range) {
        jint size = static_cast<jint>(self->size());
        if (0 <= index && index < size) {
          CTYPE old_value = std::move((*self)[index]);
...
%define %std_vector_unique_ptr(NAME, TYPE)

namespace std {
    template<> class vector<std::unique_ptr<TYPE>> {
        SWIG_STD_VECTOR_MINIMUM_INTERNAL_UNIQUE(std::unique_ptr<TYPE>, const std::unique_ptr<TYPE>&)
    };
}

%unique_ptr(TYPE);

%template(NAME) std::vector<std::unique_ptr<TYPE>>;
